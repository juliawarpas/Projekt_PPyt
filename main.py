...
def add_object():
    ...
    if not all([name, surname, location, post]):
        print("Wszystkie pola muszą być wypełnione.")
        return

    try:
        if typ == "Placówka":
            obj = Facility(name, surname, location, post, map_widget)
...
#formularz dodawania
Label(ramka_formularz, text="Imię:").grid(row=0, column=0)
entry_imie = Entry(ramka_formularz)
entry_imie.grid(row=0, column=1)

Label(ramka_formularz, text="Nazwisko:").grid(row=1, column=0)
entry_nazwisko = Entry(ramka_formularz)
entry_nazwisko.grid(row=1, column=1)

Label(ramka_formularz, text="Miejscowość:").grid(row=2, column=0)
entry_miejscowosc = Entry(ramka_formularz)
entry_miejscowosc.grid(row=2, column=1)

Label(ramka_formularz, text="Stanowisko/Rola:").grid(row=3, column=0)
